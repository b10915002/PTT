...
void Viewer::updateScreen()
{
    ...
    case boardManager->BoardState::SELECT_BOARD:
    {
        ...
        cout << left << "【看板列表】              批踢踢作業坊              看板《" << _board.getName() << "》class BoardState::SELECT_BOARD\n"
             << endl
             << "[↑][↓]選擇列  [←]回到上一層  [→]進入下一層";
        if (boardManager->users[boardManager->getCurrentUser()]->getPermissionLevel() == 3)
        {
            cout << "  [p]在當前位置開新看板 [d]刪除所選看板與子內容";
        }
        cout << endl
             << endl
    ...
    {
        Board _board = boardManager->boards[boardManager->getCurrentBoard()];
        cout << left << "【看板列表】              批踢踢作業坊              看板《" << _board.getName() << "》class BoardState::BOARD\n"
             << endl
             << "[↑][↓]選擇列  [←]回到上一層  [→]進入下一層  [d]刪除所選貼文"
             << endl
             << endl
    ...
    case boardManager->BoardState::POST:
    {
        ...
        cout << "[↑][↓]選擇列  [←]回到上一層  [1][2][3]值得推薦/給他噓聲/只加註解  [d]刪除所選推文  [e]編輯文章"
             << endl
             << left << "作者:" << _post.getAuthorID() << right << setw(85) << "看板:" << boardManager->boards[_post.getBoardID()].getName() << endl
