    ...
    while (1)
    {
        ...
        default:
            ...
            if(ch == 'y'){
                ...
                ofs << boardManager->postsJson << endl;
                ofs.close();
                boardManager->initPosts();
            }
            return;
...
void User::addPost()
{
    string boardname = boardManager->boardJson["Boards"][boardManager->getCurrentBoard()]["name"].get<string>();
    string title = "", content = "";
    system("CLS");
    cout << "發表文章於【" << boardname << "】看板\n\n輸入文章標題:";
    getline(cin,title,'\n');
    if(title.length()<1){
        getline(cin,title,'\n');
    };
    cout << "\n輸入文章內容:\n";
    getline(cin,content);
    cout << endl << endl << endl << "是否發表文章? (y/n)";
    char ch = _getch();
    if(ch == 'y'){
        boardManager->insertPost(title,content);
        boardManager->initPosts();
